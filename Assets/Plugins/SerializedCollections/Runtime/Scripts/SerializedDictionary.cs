...
namespace AYellowpaper.SerializedCollections
{
    ...
    public class SerializedDictionary<TKey, TValue> : Dictionary<TKey, TValue>, ISerializationCallbackReceiver
    {
        ...
        public new List<TKey> GetKeys() {
            List<TKey> allKeys = new List<TKey>();
            foreach (SerializedKeyValuePair<TKey, TValue> keyPair in _serializedList)
            {
                allKeys.Add(keyPair.Key);
            }
            return allKeys;
        }

        public new List<TValue> GetValue() {
            List<TValue> allValue = new List<TValue>();
            foreach (SerializedKeyValuePair<TKey, TValue> keyPair in _serializedList)
            {
                allValue.Add(keyPair.Value);
            }
            return allValue;
        }
